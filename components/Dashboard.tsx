...
import { Users, ShoppingCart, TrendingUp, AlertCircle, Clock, Calendar, CheckCircle, ArrowUpRight, Activity, Filter } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ hospitals, usageRecords, notes }) => {
  ...
  const stats = [
    ...
    { label: '進行中試用', value: activeTrials, icon: Activity, color: 'text-amber-600', bg: 'bg-amber-100', trend: '持平' },
    { label: '協商中', value: inNegotiation, icon: ArrowUpRight, color: 'text-emerald-600', bg: 'bg-emerald-100', trend: '+1' },
  ];
                              ...
                              return (
                                  <div key={note.id} className="flex gap-4 group">
                                      <div className="flex flex-col items-center">
                                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-sm border border-white ring-4 ring-slate-50 transition-all group-hover:scale-110 ${
                                              note.activityType === '會議' ? 'bg-purple-100 text-purple-600' : 
